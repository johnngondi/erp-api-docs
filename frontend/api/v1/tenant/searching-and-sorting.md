# Tenant API: Searching and Sorting

This document is for frontend developers integrating tenant-facing list endpoints.
...
## Invoice (`/tenant/billing/invoices`)
...
```http
GET /tenant/billing/invoices?filter[status]=unpaid
GET /tenant/billing/invoices?filter[status]=partially%20paid
```
...
```http
GET /tenant/billing/invoices?filter[search]=1,2,3
```
...
```http
GET /tenant/billing/invoices?filter[search]=INV100,INV200
```
...
```http
GET /tenant/billing/invoices?filter[search]=Collins
GET /tenant/billing/invoices?filter[search]=15000
```
...
```http
GET /tenant/billing/invoices?sort=due_at
GET /tenant/billing/invoices?sort=-created_at
GET /tenant/billing/invoices?sort=-total
```

### Combined examples

```http
GET /tenant/billing/invoices?filter[facility_id]=12&filter[status]=unpaid&sort=-due_at&per_page=25
GET /tenant/billing/invoices?filter[search]=INV100,INV200&sort=-id&per_page=50
GET /tenant/billing/invoices?filter[search]=Collins&filter[facility_id]=12&sort=-created_at
```

## Credit Note (`/tenant/billing/credit-notes`)
...
```http
GET /tenant/billing/credit-notes?filter[status]=applied
GET /tenant/billing/credit-notes?filter[status]=cancelled
```
...
```http
GET /tenant/billing/credit-notes?filter[search]=1,2,3
```
...
```http
GET /tenant/billing/credit-notes?filter[search]=INV100,REF200
```
...
```http
GET /tenant/billing/credit-notes?filter[search]=Collins
GET /tenant/billing/credit-notes?filter[search]=15000
```
...
```http
GET /tenant/billing/credit-notes?sort=due_at
GET /tenant/billing/credit-notes?sort=-created_at
GET /tenant/billing/credit-notes?sort=-total
```

### Combined examples

```http
GET /tenant/billing/credit-notes?filter[facility_id]=12&filter[status]=applied&sort=-due_at&per_page=25
GET /tenant/billing/credit-notes?filter[search]=INV100,REF200&sort=-id&per_page=50
GET /tenant/billing/credit-notes?filter[search]=Collins&filter[facility_id]=12&sort=-created_at
```

## Receipt (`/tenant/billing/receipts`)
...
```http
GET /tenant/billing/receipts?filter[status]=confirmed
GET /tenant/billing/receipts?filter[status]=cancelled
```
...
```http
GET /tenant/billing/receipts?filter[search]=1,2,3
```
...
```http
GET /tenant/billing/receipts?filter[search]=TRX100,TRX200
```
...
```http
GET /tenant/billing/receipts?filter[search]=Collins
GET /tenant/billing/receipts?filter[search]=15000
```
...
```http
GET /tenant/billing/receipts?sort=transaction_date
GET /tenant/billing/receipts?sort=-created_at
GET /tenant/billing/receipts?sort=-amount
```

### Combined examples

```http
GET /tenant/billing/receipts?filter[status]=confirmed&filter[payment_method_id]=1&sort=-transaction_date&per_page=25
GET /tenant/billing/receipts?filter[search]=TRX100,TRX200&sort=-id&per_page=50
GET /tenant/billing/receipts?filter[search]=Collins&filter[receiving_account_id]=3&sort=-created_at
```
...
```http
GET /leases?filter[status]=active
GET /leases?filter[status]=suspended
```
...
```http
GET /leases?filter[search]=1,2,3
```
...
```http
GET /leases?filter[search]=Collins
GET /leases?filter[search]=monthly
```
...
```http
GET /leases?sort=end_at
GET /leases?sort=-created_at
GET /leases?sort=-start_at
```

### Combined examples

```http
GET /leases?filter[facility_id]=2&filter[status]=active&sort=-end_at&per_page=25
GET /leases?filter[search]=1,2,3&sort=-id&per_page=50
GET /leases?filter[search]=Collins&filter[billing_cycle]=monthly&sort=-created_at
```

## Lease Application (`/lease-applications`)
...
```http
GET /lease-applications?filter[status]=pending
GET /lease-applications?filter[status]=approved
```
...
```http
GET /lease-applications?filter[search]=1,2,3
```
...
```http
GET /lease-applications?filter[search]=Acme
GET /lease-applications?filter[search]=Nairobi
```
...
```http
GET /lease-applications?sort=application_submitted_at
GET /lease-applications?sort=-created_at
GET /lease-applications?sort=-reviewed_at
```

### Combined examples

```http
GET /lease-applications?filter[status]=review&filter[facility_type_id]=2&sort=-application_submitted_at&per_page=25
GET /lease-applications?filter[search]=1,2,3&sort=-id&per_page=50
GET /lease-applications?filter[search]=Acme&filter[city_id]=1&sort=-created_at
```
...
## Add New Tenant Models To This Doc

When adding search/sort support for another tenant-facing model, append a new section using this template:
