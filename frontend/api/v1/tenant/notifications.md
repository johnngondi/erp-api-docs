# Notifications API — Tenant Portal
...
- `PATCH /api/v1/tenant/notifications/{notification}/read`
...
- `domain` is **not** part of this payload here and `filter[domain]` is **not** supported — a domain
  ...
  Tenant recipient. Sending `filter[domain]` returns `400` rather than a silently empty page. The
